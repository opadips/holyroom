...
- **Voice mesh** – every participant hears everyone else in real time  
- **Screen sharing** – share any monitor or window up to 4K at 60 fps, or set a custom resolution and frame rate  
...
- **HTTPS by default** – all communication (signaling and page load) is encrypted with a locally generated certificate.
...
| **Backend**      | Node.js, Express, HTTPS (self‑signed via mkcert)                           |
...
- **Node.js** ≥ 18
- **npm** (comes with Node)
- **mkcert** (for HTTPS certificates) – [download here](https://github.com/FiloSottile/mkcert/releases)
...
### 2. Generate SSL certificates (only once)
...
- On the host machine: `https://localhost:3000`
- On other devices in your network: `https://YOUR_LOCAL_IP:3000`
...
├── server/
│   └── src/index.js             ← HTTPS server, Socket.io, all event routing
└── client/
    ├── public/index.html
    └── src/
...
|----------|---------|-------------|
| `PORT` (server) | `3001` | Port on which the HTTPS server listens |
| `HTTPS` (client) | `true` | Enables HTTPS for the React dev server |
| `SSL_CRT_FILE` (client) | `../cert.pem` | Path to the SSL certificate |
| `SSL_KEY_FILE` (client) | `../key.pem` | Path to the SSL private key |

These are already configured in `start.bat` and the respective `package.json` scripts.
